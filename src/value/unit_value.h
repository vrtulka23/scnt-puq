...
class Exponent {
public:
  EXPONENT_PRECISION numerator;
  EXPONENT_PRECISION denominator;
  Exponent(): numerator(1), denominator(1) {}
  Exponent(EXPONENT_PRECISION n): numerator(n), denominator(1) {};
  Exponent(EXPONENT_PRECISION n, EXPONENT_PRECISION d): numerator(n), denominator(d) {};
...
public:
  ...
  BaseUnit() {};
  BaseUnit(EXPONENT_PRECISION n): exponent(n) {};
#ifdef EXPONENT_FRACTIONS
  BaseUnit(EXPONENT_PRECISION n, EXPONENT_PRECISION d): exponent(n,d) {};
#endif
  BaseUnit(std::string p, std::string u, EXPONENT_TYPE e): prefix(p), unit(u), exponent(e) {};
#ifdef EXPONENT_FRACTIONS
  BaseUnit(std::string p, std::string u, EXPONENT_PRECISION n, EXPONENT_PRECISION d): prefix(p), unit(u), exponent(n,d) {};
#endif
  std::string to_string();
};
...
#ifdef EXPONENT_FRACTIONS
  void append(std::string p, std::string u, EXPONENT_PRECISION n, EXPONENT_PRECISION d);
#endif
  std::string to_string();
