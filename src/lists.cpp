...
namespace puq {
  ...
  namespace lists {
    ...
    std::string prefixes(const bool json) {
      DataTable tab({{"Symbol",8},{"Name",8},{"Magnitude",11}});
      for (auto symbol: UnitPrefixOrder) {
	auto prefix = UnitPrefixList.at(symbol);
	tab.append({symbol, prefix.name, nostd::to_string(prefix.magnitude)});
      }
      return (json) ? tab.to_json() : tab.to_string();
    }

    std::string base_units(const bool json) {
    ...
    std::string derived_units(const bool json) {
    ...
    std::string constants(const bool json) {
    ...
    std::string quantities(const bool json) {
